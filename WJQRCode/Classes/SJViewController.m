...
#import "RegistDeviceViewController.h"
...
- (void)clickBarButtonItemSJButtonType:(SJButtonType)type {
    ...
    else if (type == STButtonTypeInput)
    {
        RegistDeviceViewController * vc = [[RegistDeviceViewController alloc] init];
...
//    }
    ...
    NSArray * array = [codesString componentsSeparatedByString:@","];
    NSLog(@"array = %@",array);
    
    RegistDeviceViewController * vc = [[RegistDeviceViewController alloc]init];
    vc.hidesBottomBarWhenPushed = YES;
    vc.ammeterSn = array[0];
    [self.navigationController pushViewController:vc animated:YES];
